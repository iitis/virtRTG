...
class VirtualXRay(Object):
	...
	DETECTOR_IMAGE_PRESETS = {
		"default": {
			...
			"window_center": None,
			"window_width": None,
			"exposure_fusion_enabled": False,
			"exposure_fusion_profile": "balanced",
			"exposure_fusion_strength": 0.85,
	...
	def sanitize_detector_image_defaults(cls, defaults):
		...
		return {
			...
			"exposure_fusion_enabled": bool(base.get("exposure_fusion_enabled", False)),
			"exposure_fusion_profile": str(base.get("exposure_fusion_profile", "balanced")).lower(),
			"exposure_fusion_strength": min(
				1.0,
				max(0.0, float(base.get("exposure_fusion_strength", 0.85))),
			),
